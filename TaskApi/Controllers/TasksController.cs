...
new TaskItem {
    ...
    public ActionResult<IEnumerable<TaskItem>> GetByPriority(string level) {
        ...
        if (!allowed.Contains(level, StringComparer.OrdinalIgnoreCase))
            return BadRequest(new { Message = "Доступные значения: Low, Normal, High" });
        var results = _tasks
    ...
    public ActionResult GetStats() {
        ...
        var stats = new {
            ...
            Completed = completed,
            Pending = pending,
            Completoin = total > 0 ? Math.Round((double)completed / total * 100, 1) : 0,
            ByPriority = new {
    ...
    [HttpGet("sorted")]
    public ActionResult<IEnumerable<TaskItem>> GetSorted([FromQuery] string by = "id",
